...
$(document).ready(function(){
  ...
  $('[data-toggle-tab]')
      .each(function(index, elem){
        var $elem = $(elem);
        var options = $.extend({}, { tab: '', targetEl:'li', toggleClass: 'on', eventName: 'click', duration: 0 }, parseJSON($elem.data('toggleTab')));
        if( options.tab == '' ) {
            return;
        }
        var tabBtn = $elem.find(options.tab);
        tabBtn.on(options.eventName, function(){
          var target = $(this).parents(options.targetEl);
          if (target.hasClass(options.toggleClass)) return false;
          target.toggleClass(options.toggleClass)
                .siblings()
                .removeClass(options.toggleClass);
        })
        ;
      })
  ;
  ...
  setTimeout(function() {
      ...
      /* 추가 :  20190104 */
      /**
       * snb-menu는 UI 업데이트가 끝난 뒤에 적용한다.
       */
      var snbWrap = $(".js-snbArea"),
